...
export default function AdminLoginPage() {
  ...
  useEffect(() => {
    if (user && !loggingOut) {
      if (user.role === 'admin') {
        ...
        setError('هذا الحساب ليس حساب مدير نظام (Admin). يرجى تسجيل الدخول من البوابة المخصصة لصلاحيتك.');
        setLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError('بيانات الدخول غير صحيحة أو ليس لديك صلاحية المدير.');
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-bg-dark flex items-center justify-center p-6 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md relative z-10 flex flex-col items-center">
        ...
        <div className="glass-card p-10 w-full border border-amber-500/10">
          <div className="mb-10 text-center">
            <div className="w-16 h-16 bg-amber-500/10 border border-amber-500/25 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-amber-500/20">
            ...
            <p className="text-slate-500 font-medium text-sm">
              بوابة تسجيل دخول المدير الإداري
            </p>
          </div>

          <div style={{ textAlign: 'center', marginBottom: '24px' }}>
            <span className="badge badge-amber text-xs px-3 py-1 font-bold">Admin Portal</span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" style={{ direction: 'rtl' }}>
            <div className="space-y-1 text-right">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/5 focus:border-amber-500/50 focus:bg-white/10 rounded-2xl py-4 pr-12 pl-6 outline-none transition-all text-white font-medium text-right font-sans"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-amber-600 text-white font-black uppercase tracking-[0.2em] py-5 rounded-2xl shadow-xl shadow-amber-600/20 hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-3 mt-8 cursor-pointer"
            >
              {loading ? (
          ...
          <div style={{ textAlign: 'center', fontSize: '11px', color: '#64748b', marginTop: '24px', lineHeight: '1.6' }}>
            <p>تسجيل دخول مخصص فقط للمديرين الإداريين المعتمدين.</p>
          </div>
