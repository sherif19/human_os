...
const AdminDashboard = () => {
  ...
  const t = (key, params) => {
    ...
    if (params) {
      Object.keys(params).forEach(p => {
        text = text.replace(`{{${p}}}`, params[p]).replace(`{${p}}`, params[p]);
      });
    }
    return text;
  };
  ...
  const fetchUsers = () => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      const monthsStr = translations[language]['months'] || "Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep,Oct,Nov,Dec";
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const count = allUsers.filter(u => {
          const uDate = u.createdAt?.toDate ? u.createdAt.toDate() : (u.createdAt ? new Date(u.createdAt) : null);
          return uDate && uDate.getMonth() === m;
        }).length;
  ...
  const fetchSales = () => {
    ...
    const handleError = (err) => {
      console.warn("fetchSales primary query failed (likely missing composite index), attempting fallback:", err);
      ...
      unsub = onSnapshot(fallbackQ, (snapshot) => {
        const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        data.sort((a, b) => {
          const valA = a.createdAt?.toDate ? a.createdAt.toDate().getTime() : (a.createdAt ? new Date(a.createdAt).getTime() : 0);
          const valB = b.createdAt?.toDate ? b.createdAt.toDate().getTime() : (b.createdAt ? new Date(b.createdAt).getTime() : 0);
          return valB - valA;
        });
        setSales(data);
      }, (fallbackErr) => {
        console.error("fetchSales fallback query also failed:", fallbackErr);
        setError(t('admin.errorSyncSales') || "Error syncing sales data");
      });
    };

    unsub = onSnapshot(q, handleSnapshot, handleError);
    return () => {
      if (unsub) unsub();
    };
  };
  ...
  const handleApproveSubscription = async (payment) => {
    ...
    try {
      ...
      if (currentExpires) {
        const currentMs = currentExpires.toDate ? currentExpires.toDate().getTime() : (currentExpires ? new Date(currentExpires).getTime() : 0);
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', editingUser.id), {
        ...
        expiresAt: expiresAt,
        isTrial: false,
        stability: Number(editingUser.stability !== undefined ? editingUser.stability : 92),
        streak: Number(editingUser.streak !== undefined ? editingUser.streak : 12),
        archetype: editingUser.archetype || 'The Strategist',
        archetypeAr: editingUser.archetypeAr || 'الاستراتيجي',
        primaryDriver: editingUser.primaryDriver || 'Logic & Efficiency',
        primaryDriverAr: editingUser.primaryDriverAr || 'المنطق والكفاءة',
        intelligenceInsight: editingUser.intelligenceInsight || 'System analysis detects a recurring avoidance pattern during social conflicts.',
        intelligenceInsightAr: editingUser.intelligenceInsightAr || 'يكشف تحليل النظام عن نمط تجنب متكرر أثناء النزاعات الاجتماعية.',
        protocol01: editingUser.protocol01 || 'Execute a 10s Pause during conflict.',
        protocol01Ar: editingUser.protocol01Ar || 'نفذ توقفاً لمدة 10 ثوانٍ أثناء النزاع.',
        protocol02: editingUser.protocol02 || 'Record thoughts in Journal immediately.',
        protocol02Ar: editingUser.protocol02Ar || 'سجل الأفكار في السجل العصبي فوراً.',
        growthProtocol: editingUser.growthProtocol || '"Phase I focus should be on Social Fluidity. Your current \'Analytical\' dominance is high-performing in isolation but creates friction in collaborative neural streams."',
        growthProtocolAr: editingUser.growthProtocolAr || '"يجب أن يكون تركيز المرحلة الأولى على السيولة الاجتماعية. سيادتك \"التحليلية\" الحالية عالية الأداء في العزلة ولكنها تخلق احتكاكاً في التيارات العصبية التعاونية."',
        strengths: editingUser.strengths || 'Complex System Mapping, Strategic Detachment, Extreme Focus',
        strengthsAr: editingUser.strengthsAr || 'تخطيط الأنظمة المعقدة, الانفصال الاستراتيجي, التركيز الشديد',
        weaknesses: editingUser.weaknesses || 'Tactical Spontaneity, Emotional Synchrony, Baseline Consistency',
        weaknessesAr: editingUser.weaknessesAr || 'العفوية التكتيكية, التزامن العاطفي, الاتساق الأساسي',
        anxiety: Number(editingUser.anxiety !== undefined ? editingUser.anxiety : 12),
        diagnosticFocus: Number(editingUser.diagnosticFocus !== undefined ? editingUser.diagnosticFocus : 88),
        confidence: Number(editingUser.confidence !== undefined ? editingUser.confidence : 65),
        discipline: Number(editingUser.discipline !== undefined ? editingUser.discipline : 48),
        emotional: Number(editingUser.emotional !== undefined ? editingUser.emotional : 75),
        charisma: Number(editingUser.charisma !== undefined ? editingUser.charisma : 50),
        leadership: Number(editingUser.leadership !== undefined ? editingUser.leadership : 60),
        consistency: Number(editingUser.consistency !== undefined ? editingUser.consistency : 45),
        focus: Number(editingUser.focus !== undefined ? editingUser.focus : 85),
        social: Number(editingUser.social !== undefined ? editingUser.social : 40),
        selfWorth: Number(editingUser.selfWorth !== undefined ? editingUser.selfWorth : 55),
        empathy: Number(editingUser.empathy !== undefined ? editingUser.empathy : 70)
      }, { merge: true });
  ...
  const handleRejectAppointment = async (apptId) => {
    if (!window.confirm(isRTL ? "هل أنت متأكد من رفض هذا الموعد؟" : "Are you sure you want to reject this appointment?")) return;
    try {
      if (currentUser?.uid.startsWith('demo-')) {
        const stored = localStorage.getItem('humanos_demo_appointments');
        if (stored) {
      ...
      alert(isRTL ? "تم رفض الموعد" : "Appointment rejected.");
    } catch (err) {
  ...
  const getSubscriptionStatus = (user) => {
    ...
    const ts = user.expiresAt;
    const expiresMs = ts?.toDate ? ts.toDate().getTime() : (ts ? new Date(ts).getTime() : 0);
    if (Date.now() > expiresMs) return { expired: true, daysLeft: 0 };
    return { expired: false, daysLeft: Math.max(1, Math.ceil((expiresMs - Date.now()) / 86400000)) };
  };
        ...
        <div>
          <h2 style={{ fontSize: '24px', fontWeight: '900', color: 'var(--text)' }}>
            {activeTab === 'users' ? t('admin.usersTitle') :
             ...
             activeTab === 'appointments' ? (isRTL ? 'حجز المواعيد' : 'Appointments') :
             t('admin.statsTitle')}
          </h2>
          <p style={{ color: 'var(--text2)', fontSize: '14px' }}>
            {activeTab === 'users' ? t('admin.usersDesc') :
             ...
             activeTab === 'appointments' ? (isRTL ? 'إدارة وتأكيد مواعيد العملاء والتحكم في حالتها.' : 'Manage and verify client appointments.') :
             t('admin.statsDesc')}
          </p>
                      ...
                      <tr key={pay.id} style={{ borderBottom: '1px solid var(--line)' }}>
                        <td style={{ padding: '12px 20px' }}>
                          ...
                          <div style={{ fontSize: '10px', color: 'var(--text3)', marginTop: '2px', fontFamily: 'var(--mono)' }}>
                            {formatUserDate(pay.createdAt, language, true)}
                          </div>
                        </td>
                    ...
                    sales.filter(s => (s.customerName || '').toLowerCase().includes(searchTerm.toLowerCase())).map(sale => (
                      <tr key={sale.id} style={{ borderBottom: '1px solid var(--line)' }}>
                        ...
                        <td style={{ padding: '16px 20px', color: 'var(--text2)', fontSize: '12px' }}>
                          {formatUserDate(sale.createdAt, language)}
                        </td>
        ...
        <div className="card" style={{ padding: '0', overflow: 'hidden', marginBottom: '24px' }}>
          <div style={{ padding: '20px', borderBottom: '1px solid var(--line)' }}>
            <h3 style={{ fontSize: '16px', fontWeight: '800', margin: 0 }}>
              {isRTL ? 'طلبات حجز المواعيد' : 'Appointment Requests'}
            </h3>
            <p style={{ fontSize: '12px', color: 'var(--text3)', margin: '4px 0 0 0' }}>
              {isRTL ? 'مراجعة المواعيد التي تم طلبها من العملاء والموافقة عليها أو رفضها.' : 'Review, approve, or reject scheduled client appointment requests.'}
            </p>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid var(--line)' }}>
                  ...
                  <th style={{ padding: '12px 20px', textAlign: 'start', fontSize: '11px', color: 'var(--text2)', fontWeight: '700', textTransform: 'uppercase' }}>
                    {isRTL ? 'التاريخ والتوقيت' : 'Requested Date & Time'}
                  </th>
                  <th style={{ padding: '12px 20px', textAlign: 'start', fontSize: '11px', color: 'var(--text2)', fontWeight: '700', textTransform: 'uppercase' }}>
                    {isRTL ? 'الملاحظات' : 'Notes / Purpose'}
                  </th>
                  <th style={{ padding: '12px 20px', textAlign: 'center', fontSize: '11px', color: 'var(--text2)', fontWeight: '700', textTransform: 'uppercase' }}>
                    {isRTL ? 'الحالة' : 'Status'}
                  </th>
              ...
              <tbody>
                {appointmentsLoading ? (
                  <tr>
                    <td colSpan="5" style={{ padding: '30px', textAlign: 'center', color: 'var(--text3)', fontSize: '12px' }}>
                  ...
                  <tr>
                    <td colSpan="5" style={{ padding: '30px', textAlign: 'center', color: 'var(--text3)', fontSize: '12px' }}>
                      {isRTL ? 'لا توجد طلبات مواعيد حالياً.' : 'No appointment requests found.'}
                    </td>
                  </tr>
                ) : (
                  appointments.map((appt) => (
                    <tr key={appt.id} style={{ borderBottom: '1px solid var(--line)' }}>
                      <td style={{ padding: '12px 20px' }}>
                        <div style={{ fontWeight: '700', color: 'var(--text)' }}>{appt.userName}</div>
                        <div style={{ fontSize: '11px', color: 'var(--text3)', marginTop: '2px' }}>{appt.userEmail}</div>
                      </td>
                      <td style={{ padding: '12px 20px', fontSize: '12px', fontWeight: '600', color: 'var(--text2)', whiteSpace: 'nowrap' }}>
                        <div>📅 {appt.date}</div>
                      ...
                      <td style={{ padding: '12px 20px', textAlign: 'center' }}>
                        <span style={{
                          fontSize: '10px',
                          padding: '4px 10px',
                          background: appt.status === 'approved' ? 'rgba(16, 185, 129, 0.1)' : appt.status === 'pending' ? 'rgba(245, 158, 11, 0.1)' : 'rgba(239, 68, 68, 0.1)',
                          color: appt.status === 'approved' ? 'var(--green)' : appt.status === 'pending' ? 'var(--amber)' : 'var(--red)',
                          border: `1px solid ${appt.status === 'approved' ? 'rgba(16, 185, 129, 0.2)' : appt.status === 'pending' ? 'rgba(245, 158, 11, 0.2)' : 'rgba(239, 68, 68, 0.2)'}`,
                          borderRadius: '20px',
                          fontWeight: '700',
                          display: 'inline-block'
                        }}>
                          {appt.status === 'approved' ? (isRTL ? 'مقبول' : 'Approved') : appt.status === 'pending' ? (isRTL ? 'معلق' : 'Pending') : (isRTL ? 'مرفوض' : 'Rejected')}
                        </span>
                      </td>
                      <td style={{ padding: '12px 20px', textAlign: 'center' }}>
                        {appt.status === 'pending' ? (
                        ...
                        {user.isTrial ? (() => {
                          ...
                          <div>
                            ...
                            {user.expiresAt && (() => {
                              ...
                              return <div style={{ color: 'var(--green)', fontSize: '11px', marginTop: '4px', fontWeight: '700' }}>⏰ {ss.daysLeft} {t('admin.daysLeft')}</div>;
                            })()}
                            ...
                            <div style={{ fontSize: '10px', color: (user.devices?.length || 0) >= 2 ? 'var(--amber)' : 'var(--text3)', marginTop: '4px', display: 'flex', alignItems: 'center', gap: '3px' }}>
                              <Smartphone size={9} />
                              {user.devices?.length || 0} {t('admin.devicesLabel')}
                            </div>
                          </div>
                        )}
                      </td>
      ...
      {showEditModal && editingUser && (
        <div className="modal-backdrop" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="card" style={{ width: '100%', maxWidth: '500px', margin: 0, animation: 'scaleUp 0.3s ease' }}>
            ...
            {/* Modal Tabs */}
            <div style={{ display: 'flex', gap: '10px', marginBottom: '15px', borderBottom: '1px solid var(--line)', paddingBottom: '10px' }}>
              <button 
                type="button" 
                onClick={() => setEditModalTab('general')} 
                className="btn btn-sm"
                style={{ flex: 1, background: editModalTab === 'general' ? 'var(--accent)' : 'rgba(255,255,255,0.03)', color: editModalTab === 'general' ? '#white' : 'var(--text3)', border: '1px solid var(--line)', borderRadius: '8px', cursor: 'pointer', padding: '6px 12px', fontSize: '12px', fontWeight: 'bold' }}
              >
                {isRTL ? 'معلومات عامة' : 'General Info'}
              </button>
              <button 
                type="button" 
                onClick={() => setEditModalTab('neural')} 
                className="btn btn-sm"
                style={{ flex: 1, background: editModalTab === 'neural' ? 'var(--accent)' : 'rgba(255,255,255,0.03)', color: editModalTab === 'neural' ? '#white' : 'var(--text3)', border: '1px solid var(--line)', borderRadius: '8px', cursor: 'pointer', padding: '6px 12px', fontSize: '12px', fontWeight: 'bold' }}
              >
                {isRTL ? 'البيانات العصبية' : 'Neural Metrics'}
              </button>
            </div>
            ...
            <form onSubmit={handleUpdateUser}>
              ...
              {editModalTab === 'neural' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '350px', overflowY: 'auto', paddingRight: '5px', marginBottom: '20px' }} className="no-scrollbar">
                  {/* Basic Metrics */}
                  <div className="grid-2" style={{ gap: '12px' }}>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'الاستقرار (%)' : 'Stability (%)'}</label>
                      <input 
                        className="form-control" 
                        type="number" 
                        min="0" 
                        max="100" 
                        value={editingUser.stability !== undefined ? editingUser.stability : 92} 
                        onChange={e => setEditingUser({ ...editingUser, stability: Number(e.target.value) })} 
                      />
                    </div>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'النشاط (أيام)' : 'Streak (Days)'}</label>
                      <input 
                        className="form-control" 
                        type="number" 
                        min="0" 
                        value={editingUser.streak !== undefined ? editingUser.streak : 12} 
                  ...
                  <div className="grid-2" style={{ gap: '12px' }}>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'النموذج الأصلي (EN)' : 'Archetype (EN)'}</label>
                      <input 
                        className="form-control" 
                        type="text" 
                        value={editingUser.archetype || 'The Strategist'} 
                        onChange={e => setEditingUser({ ...editingUser, archetype: e.target.value })} 
                      />
                    </div>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'النموذج الأصلي (AR)' : 'Archetype (AR)'}</label>
                      <input 
                        className="form-control" 
                        type="text" 
                        value={editingUser.archetypeAr || 'الاستراتيجي'} 
                        onChange={e => setEditingUser({ ...editingUser, archetypeAr: e.target.value })} 
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid-2" style={{ gap: '12px' }}>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'المحرك الأساسي (EN)' : 'Primary Driver (EN)'}</label>
                      <input 
                        className="form-control" 
                        type="text" 
                        value={editingUser.primaryDriver || 'Logic & Efficiency'} 
                        onChange={e => setEditingUser({ ...editingUser, primaryDriver: e.target.value })} 
                      />
                    </div>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'المحرك الأساسي (AR)' : 'Primary Driver (AR)'}</label>
                      <input 
                        className="form-control" 
                        type="text" 
                        value={editingUser.primaryDriverAr || 'المنطق والكفاءة'} 
                        onChange={e => setEditingUser({ ...editingUser, primaryDriverAr: e.target.value })} 
                      />
                    </div>
                  </div>
                  ...
                  <div className="field">
                    <label className="field-label">{isRTL ? 'رؤية الذكاء العصبي (EN)' : 'Neural Insight (EN)'}</label>
                    <textarea 
                      className="form-control" 
                      style={{ height: '60px', resize: 'vertical' }}
                      value={editingUser.intelligenceInsight || 'System analysis detects a recurring avoidance pattern during social conflicts.'} 
                      onChange={e => setEditingUser({ ...editingUser, intelligenceInsight: e.target.value })} 
                    />
                  </div>
                  <div className="field">
                    <label className="field-label">{isRTL ? 'رؤية الذكاء العصبي (AR)' : 'Neural Insight (AR)'}</label>
                    <textarea 
                      className="form-control" 
                      style={{ height: '60px', resize: 'vertical' }}
                      value={editingUser.intelligenceInsightAr || 'يكشف تحليل النظام عن نمط تجنب متكرر أثناء النزاعات الاجتماعية.'} 
                      onChange={e => setEditingUser({ ...editingUser, intelligenceInsightAr: e.target.value })} 
                    />
                  </div>
                  ...
                  <div className="grid-2" style={{ gap: '12px' }}>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'بروتوكول 1 (EN)' : 'Protocol 01 (EN)'}</label>
                      <input 
                        className="form-control" 
                        type="text" 
                        value={editingUser.protocol01 || 'Execute a 10s Pause during conflict.'} 
                        onChange={e => setEditingUser({ ...editingUser, protocol01: e.target.value })} 
                      />
                    </div>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'بروتوكول 1 (AR)' : 'Protocol 01 (AR)'}</label>
                      <input 
                        className="form-control" 
                        type="text" 
                        value={editingUser.protocol01Ar || 'نفذ توقفاً لمدة 10 ثوانٍ أثناء النزاع.'} 
                        onChange={e => setEditingUser({ ...editingUser, protocol01Ar: e.target.value })} 
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid-2" style={{ gap: '12px' }}>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'بروتوكول 2 (EN)' : 'Protocol 02 (EN)'}</label>
                      <input 
                        className="form-control" 
                        type="text" 
                        value={editingUser.protocol02 || 'Record thoughts in Journal immediately.'} 
                        onChange={e => setEditingUser({ ...editingUser, protocol02: e.target.value })} 
                      />
                    </div>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'بروتوكول 2 (AR)' : 'Protocol 02 (AR)'}</label>
                      <input 
                        className="form-control" 
                        type="text" 
                        value={editingUser.protocol02Ar || 'سجل الأفكار في السجل العصبي فوراً.'} 
                        onChange={e => setEditingUser({ ...editingUser, protocol02Ar: e.target.value })} 
                      />
                    </div>
                  </div>

                  {/* Growth Protocol Recommendation */}
                  <div className="field">
                    <label className="field-label">{isRTL ? 'توصية بروتوكول النمو (EN)' : 'Growth Protocol (EN)'}</label>
                    <textarea 
                      className="form-control" 
                      style={{ height: '60px', resize: 'vertical' }}
                      value={editingUser.growthProtocol || '"Phase I focus should be on Social Fluidity. Your current \'Analytical\' dominance is high-performing in isolation but creates friction in collaborative neural streams."'} 
                      onChange={e => setEditingUser({ ...editingUser, growthProtocol: e.target.value })} 
                    />
                  </div>
                  <div className="field">
                    <label className="field-label">{isRTL ? 'توصية بروتوكول النمو (AR)' : 'Growth Protocol (AR)'}</label>
                    <textarea 
                      className="form-control" 
                      style={{ height: '60px', resize: 'vertical' }}
                      value={editingUser.growthProtocolAr || '"يجب أن يكون تركيز المرحلة الأولى على السيولة الاجتماعية. سيادتك \"التحليلية\" الحالية عالية الأداء في العزلة ولكنها تخلق احتكاكاً في التيارات العصبية التعاونية."'} 
                      onChange={e => setEditingUser({ ...editingUser, growthProtocolAr: e.target.value })} 
                    />
                  </div>

                  {/* Strengths & Weaknesses (Comma separated) */}
                  <div className="grid-2" style={{ gap: '12px' }}>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'نقاط القوة (EN - مفصولة بفواصل)' : 'Strengths (EN - Comma separated)'}</label>
                      <input 
                        className="form-control" 
                        type="text" 
                        value={editingUser.strengths || 'Complex System Mapping, Strategic Detachment, Extreme Focus'} 
                        onChange={e => setEditingUser({ ...editingUser, strengths: e.target.value })} 
                      />
                    </div>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'نقاط القوة (AR - مفصولة بفواصل)' : 'Strengths (AR - Comma separated)'}</label>
                      <input 
                        className="form-control" 
                        type="text" 
                        value={editingUser.strengthsAr || 'تخطيط الأنظمة المعقدة, الانفصال الاستراتيجي, التركيز الشديد'} 
                        onChange={e => setEditingUser({ ...editingUser, strengthsAr: e.target.value })} 
                      />
                    </div>
                  </div>
                  <div className="grid-2" style={{ gap: '12px' }}>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'نقاط الضعف (EN - مفصولة بفواصل)' : 'Vulnerabilities (EN - Comma separated)'}</label>
                      <input 
                        className="form-control" 
                        type="text" 
                        value={editingUser.weaknesses || 'Tactical Spontaneity, Emotional Synchrony, Baseline Consistency'} 
                        onChange={e => setEditingUser({ ...editingUser, weaknesses: e.target.value })} 
                      />
                    </div>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'نقاط الضعف (AR - مفصولة بفواصل)' : 'Vulnerabilities (AR - Comma separated)'}</label>
                      <input 
                        className="form-control" 
                        type="text" 
                        value={editingUser.weaknessesAr || 'العفوية التكتيكية, التزامن العاطفي, الاتساق الأساسي'} 
                        onChange={e => setEditingUser({ ...editingUser, weaknessesAr: e.target.value })} 
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid-2" style={{ gap: '12px' }}>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'القلق (%)' : 'Anxiety (%)'}</label>
                      <input 
                        className="form-control" 
                        type="number" 
                        min="0" 
                        max="100" 
                        value={editingUser.anxiety !== undefined ? editingUser.anxiety : 12} 
                        onChange={e => setEditingUser({ ...editingUser, anxiety: Number(e.target.value) })} 
                      />
                    </div>
                    <div className="field">
                      <label className="field-label">{isRTL ? 'التركيز (%)' : 'Focus (%)'}</label>
                      <input 
                        className="form-control" 
                        type="number" 
                        min="0" 
                        max="100" 
                        value={editingUser.diagnosticFocus !== undefined ? editingUser.diagnosticFocus : 88} 
                        onChange={e => setEditingUser({ ...editingUser, diagnosticFocus: Number(e.target.value) })} 
                      />
                    </div>
                  </div>
                  ...
                  <div style={{ borderTop: '1px solid var(--line)', marginTop: '10px', paddingTop: '10px' }}>
                    <h5 style={{ fontSize: '12px', fontWeight: '800', marginBottom: '10px', color: 'var(--accent)' }}>
                      {isRTL ? 'قيم خريطة رادار الذكاء (0 - 100)' : 'Intelligence Radar Values (0 - 100)'}
                    </h5>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      {[
                        { key: 'confidence', label: 'Confidence / الثقة', defaultVal: 65 },
                        { key: 'discipline', label: 'Discipline / الانضباط', defaultVal: 48 },
                        { key: 'emotional', label: 'EQ / الذكاء العاطفي', defaultVal: 75 },
                        { key: 'charisma', label: 'Charisma / الكاريزما', defaultVal: 50 },
                        { key: 'leadership', label: 'Leadership / القيادة', defaultVal: 60 },
                        { key: 'consistency', label: 'Consistency / الاتساق', defaultVal: 45 },
                        { key: 'focus', label: 'Focus / التركيز', defaultVal: 85 },
                        { key: 'social', label: 'Social / اجتماعي', defaultVal: 40 },
                        { key: 'selfWorth', label: 'Self Worth / الذات', defaultVal: 55 },
                        { key: 'empathy', label: 'Empathy / التعاطف', defaultVal: 70 }
                      ].map(item => (
                        <div key={item.key} className="field">
                          <label className="field-label" style={{ fontSize: '10px' }}>{item.label}</label>
                          <input 
                            className="form-control" 
                            type="number" 
                            min="0" 
                            max="100" 
                            value={editingUser[item.key] !== undefined ? editingUser[item.key] : item.defaultVal} 
                            onChange={e => setEditingUser({ ...editingUser, [item.key]: Number(e.target.value) })} 
                          />
                        </div>
                      ))}
                    </div>
